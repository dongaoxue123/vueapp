              ...
              <div style="padding: 20px;">
                ...
                <van-cell-group style="margin-top: 20px;">
                  <!-- <van-field
                    ...
                    @input="changeTalkup"
                  /> -->
                  <!-- <div style="display: flex;justify-content:flex-start;align-items: center;padding-left:14px"> -->
                    <h4>禁言原因</h4>
                    <van-dropdown-menu>
                    <van-dropdown-item style="color: #585858;font-size:14px;" v-model="setvaluedone" :options="option2" @change="changeValuedone(setvaluedone)"/>
                    </van-dropdown-menu>
                  <!-- </div> -->
                </van-cell-group>
                <div class="savebut">
                  ...
                  <van-button @click="popElite" style="border-radius: 4px 4px 4px 4px;border-color: #f89f8d;color: #f89f8d;" size="mini" plain type="info">设为精华</van-button>
                    ...
                    <div style="padding: 20px;">
                      ...
                      <van-radio-group v-model="radio" @change="changeName">
                        ...
                        <van-radio style="margin-top: 20px;" name="3">内容丰富</van-radio>
                        <van-radio style="margin-top: 20px;" name="4">当下流行</van-radio>
                      </van-radio-group>
                      <div class="savebut">
                  ...
                  <van-button @click="popDel" style="border-radius: 4px 4px 4px 4px;border-color: #eaeaea;color: #585858;" size="mini" plain type="info">删除</van-button>
                    ...
                    >
                      <div style="padding: 20px;">
                        <h3>是否将以下主题删除</h3>
                        <span style="color:pink;font-size:14px">【{{type}}】</span>&nbsp;&nbsp;&nbsp;<span style="font-size:14px;color:#333">{{name}}</span>
                        <van-radio-group v-model="radio" @change="changedel">
          ...
          <div v-for="(item, index) in pinglunList" style="padding: 5px;" class="pinglunblock">
            ...
            <div style="margin-left: 5px;width: 100%;">
              <div class="pinglun">
                ...
                <div @click="popdoneTextpinglun" style="border: 1px solid red;border-radius: 5px 5px 5px 5px;font-size:12px; color:red;width: 30px;text-align: center;">禁言</div>
                  <van-popup
                    v-model="isShowDonepinglun"
                    round
                    position="bottom"
                    :style="{ height: '65%' }"
                  >
                  <div style="padding: 20px;">
                    <h3>是否将禁言</h3>
                    <span style="color:pink;font-size:15px;font-weight: 600;">{{item.username}}</span>
                    <h4>禁言天数</h4>
                    <van-radio-group v-model="radio" @change="changeTalk">
                      <van-radio style="margin-top: 20px;" name="0">三天</van-radio>
                      <van-radio style="margin-top: 20px;" name="1">七天</van-radio>
                    ...
                    <div class="savebut">
                      <van-button @click="canel3"  style="padding-left: 20px;padding-right: 20px;font-size: 15px;" type="default">取消</van-button>
                      <van-button @click="sureTalkpinglun(item)" style="padding-left: 20px;padding-right: 20px;" type="info">确认</van-button>
                    </div>
                  </div>
                </van-popup>
              </div>
...
export default {
  ...
  data () {
    return {
      ...
      setvaluedone:0,
      option2:[
      { text: '因敏感词被系统禁言', value: 0 },
      { text: '不同意见，被版主禁言', value: 1 },
      { text: '严重违规被系统永久禁言', value: 2 }
      ],
      checked: true,
      name: '',
      contentList: [],
  ...
  methods: {
    sureTalkpinglun (item) {
      axios.defaults.headers.common['authToken'] = localStorage.name
      doPost(`${API_HOST}/api/club/tiezi/setGag`, {createuserid:item.createuserid, delReason:this.changeTalkuptxt, status: this.changeTalkTxt}).then(res => {
        if (res) {
          console.log(res.data)
          Toast.success('设置成功')
          this.isShowDonepinglun = false
        }
      })
    },
    popdoneTextpinglun () {
      this.isShowDonepinglun = true
    },
    changeValuedone(setvaluedone){
      console.log(setvaluedone)
      this.changeTalkuptxt = setvaluedone
    },
    showimgpop (item) {
      this.show = true
